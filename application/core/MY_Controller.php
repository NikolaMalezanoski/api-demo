...
abstract class API_Controller extends REST_Controller {
	...
	public function protectedByJWT(){
		$this->load->model('Responses/ResponseFail');
		$this->load->model('JWTAccessToken');
		...
		if (!(!empty($is_valid_token) AND $is_valid_token['status'] === TRUE)) {
			...
			$this->ResponseFail->errors = ['error' => $is_valid_token['message']];
			$this->response($this->ResponseFail->getDataArray(), $this::HTTP_UNAUTHORIZED, true);
		}
		/** @var JWTAccessToken */
		$JWTTokenData = $this->authorization_token->userData();
		$this->JWTAccessToken->setData($JWTTokenData);
	}
}
/**
 * CodeIgniter Rest Controller
...
abstract class REST_Controller extends CI_Controller {
     ...
     */
    public function response($data = NULL, $http_code = NULL, $closeResponse = false)
    {
        ob_start();
